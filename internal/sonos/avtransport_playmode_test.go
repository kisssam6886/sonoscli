...
<RecQualityMode>NOT_IMPLEMENTED</RecQualityMode>
	...
	if settings.PlayMode != PlayModeShuffle {
		t.Fatalf("PlayMode = %q, want %q", settings.PlayMode, PlayModeShuffle)
	}
	if settings.RecQualityMode != "NOT_IMPLEMENTED" {
		t.Fatalf("RecQualityMode = %q, want NOT_IMPLEMENTED", settings.RecQualityMode)
	}
}
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
<s:Body>
<u:SetPlayModeResponse xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"></u:SetPlayModeResponse>
</s:Body>
</s:Envelope>`), nil
			})
			c := &Client{IP: "192.0.2.1", HTTP: &http.Client{Timeout: time.Second, Transport: rt}}
			...
			if requestedMode != string(tt.mode) {
				t.Fatalf("requested mode = %q, want %q", requestedMode, tt.mode)
			}
		})
	}
}
